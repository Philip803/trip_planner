const mapboxgl = require("mapbox-gl");
const buildMarker = require("./marker").buildMarker;

mapboxgl.accessToken = "pk.eyJ1IjoicGhpbGlwODAzIiwiYSI6ImNqZXp0OTZteDBmeWMycW9ham10bWFwZGgifQ.eAz3_XBnKQLumSLnPNApoQ";

const map = new mapboxgl.Map({
  container: "map",
  center: [-74.009, 40.705], // FullStack NY coordinates; alternatively, use [-87.6354, 41.8885] for Chicago
  zoom: 12, // starting zoom
  style: "mapbox://styles/mapbox/streets-v10" // mapbox has lots of different map styles available.
});

// new mapboxgl.Marker(markerDomEl).setLngLat([-74.009, 40.705]).addTo(map); // [-87.6354, 41.8885] for Chicago



const fullstackMarker = buildMarker("activities", [-74.009151, 40.705086]); // or [-87.6354, 41.8885]
const hotels = buildMarker("hotels", [-74.019151, 40.715086]);
const restaurants = buildMarker("restaurants", [-74.12312, 40.802134]);

hotels.addTo(map);
fullstackMarker.addTo(map);
restaurants.addTo(map);

